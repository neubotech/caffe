...
template <typename Dtype>
class CuDNNConvolutionLayerTest2 : public ::testing::Test {
 protected:
  CuDNNConvolutionLayerTest2()
  ...
  virtual ~CuDNNConvolutionLayerTest2() {
    delete blob_bottom_;
...
TYPED_TEST_CASE(CuDNNConvolutionLayerTest2, TestDtypes);

TYPED_TEST(CuDNNConvolutionLayerTest2, TestSimpleConvolutionCuDNN) {
...
TYPED_TEST(CuDNNConvolutionLayerTest2, TestConvolutionStrideWeightsCuDNN) {
  // We will simply see if the convolution layer carries out averaging well.
  Caffe::set_mode(Caffe::GPU);
  LayerParameter layer_param;
  ConvolutionParameter* convolution_param =
  ...
  convolution_param->set_stride(2);
  convolution_param->set_num_output(4);
  convolution_param->mutable_weight_filler()->set_type("constant_stride");
  convolution_param->mutable_weight_filler()->set_value(1.);
  convolution_param->mutable_weight_filler()->set_stride(3);
  convolution_param->mutable_weight_filler()->set_start(0);
  convolution_param->mutable_weight_filler()->set_end(100);
  convolution_param->mutable_bias_filler()->set_type("constant");
  convolution_param->mutable_bias_filler()->set_value(0.1);
  shared_ptr<Layer<TypeParam> > layer(
      new CuDNNConvolutionLayer<TypeParam>(layer_param));
