...
__global__ void col2row_gpu_kernel(const int n, const Dtype* data_in,
...
void col2row_gpu(const Dtype* data_in, const int w,
  ...
  col2row_gpu_kernel<Dtype><<<CAFFE_GET_BLOCKS(num_kernels),
...
template void col2row_gpu<float>(const float* data_in, const int w,
    const int h, const int m, const int n_g,
    float* data_out);
template void col2row_gpu<double>(const double* data_in, const int w,
    const int h, const int m, const int n_g,
    double* data_out);

template <typename Dtype>
__global__ void row2col_gpu_kernel(const int n, const Dtype* data_in,
    const int w, const int h, const int m, //const int n_g,
...
void row2col_gpu(const Dtype* data_in, const int w,
  ...
  // NOLINT_NEXT_LINE(whitespace/operators)
  row2col_gpu_kernel<Dtype><<<CAFFE_GET_BLOCKS(num_kernels),
                             CAFFE_CUDA_NUM_THREADS>>>(
       num_kernels, data_in,
...
// Explicit instantiation
template void row2col_gpu<float>(const float* data_in, const int w,
    const int h, const int m, //const int n_g,
    const int ofst, int topi, float* data_out);
template void row2col_gpu<double>(const double* data_in, const int w,
    const int h, const int m, //const int n_g,
